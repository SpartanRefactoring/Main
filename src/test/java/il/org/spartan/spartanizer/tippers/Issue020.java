...
/** @author Yossi Gil
 * @since 2016 */
@FixMethodOrder(MethodSorters.NAME_ASCENDING) @SuppressWarnings({ "static-method", "javadoc" }) @Ignore public class Issue020 {
  /** Correct way of trimming does not change */
  @Test public void constructorParameterRenaming_00() {
    trimmingOf("class A {" + "int x;" + "A(int y) {" + "this.x = y;" + "}}").gives("class A {" + "int x;" + "A(int x) {" + "this.x = x;" + "}}")
        .stays();
  }
  ...
  @Test public void constructorParameterRenaming_03() {
    trimmingOf("class A {" + "int x;" + "A(int y, int k) {" + "this.x = k;" + "}}")
        .gives("class A {" + "int x;" + "A(int y, int x) {" + "this.x = x;" + "}}").stays();
  }
