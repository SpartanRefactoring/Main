...
public class ReturnsHoldsForSomeTest {
  @Test public void _a() {
    trimmingOf(" for (final UserDefinedTipper<Statement> ¢ : tippers) if (¢.canTip(s)) return true; return false;")
        .withTipper(Block.class, new ReturnHoldsForSome()).gives("return tippers.stream().anyMatch(¢ -> ¢.canTip(s));");
  }

  @Test public void a() {
    trimmingOf("for (final Object ¢ : os)  if (¢.equals(best))   return true; return false;")//
        .withTipper(Block.class, new ReturnHoldsForSome())//
