...
/** Unit test for
 * {@link LocalVariableIntializedStatementTerminatingScope}Inlining of
...
public class Issue0364 {
  @Test public void emptyInitializer() {
    topDownTrimming("Object[] os = {};S.out.println(os);").givesEither("S.out.println((new Object[] {}));", "S.out.println(new Object[] {});");
  }

  @Test public void notTerminating() {
    topDownTrimming("void f() { String[] x = {\"\"}; g(x); h();}")//
        .gives("void f() { g(new String[] {\"\"}); h();}");
  }

  @Test public void notTerminatingRealWorld() {
    topDownTrimming("@Test public void fz() throws E { String[] cmd = {S.p(\"java.home\") + F.s + \"bin\""
  ...
  @Test public void realLifeExample() {
    topDownTrimming("if (opterr) { final Object[] msgArgs = { progname, Character.valueOf((char) c) + \"\" };"
