...
public class Issue0456 {
  @Test public void a() {
    trimmingOf("{ long q = (r & 1) != 0 ? d(x, r) : (x >>> 1) / (r >>> 1); long r = x - r * q; b[--i] = C.f((int) r, r); x = q; for (; x > 0; x /= r)   b[--i] = C.f((int) (x % r), r);}"
    ).gives(
  ...
  /** Automatically generated on Thu-Mar-16-12:25:21-IST-2017, copied by Yossi */
  @Test public void d() {
     trimmingOf("{ long q = (r & 1) != 0 ? d(x, r) : (x >>> 1) / (r >>> 1), r = x - q * r; b[--i] = C.f((int) r, r); x = q; for (; x > 0; x /= r) b[--i] = C.f((int) (x % r), r); }") //
    .stays() //
    ;
  }
