...
public class Issue0222 {
  @Test public void chocolate1() {
    topDownTrimming("L<E> os(I x){if(x==null)return null;int y=x;f(++y); L<E> $=new A<>();$.a(l(x));$.a(r(x));if(x.h()) $.d(s.es(x));return $;}")//
        .stays();
  }

  @Test public void chocolate2() {
    topDownTrimming("tc¢(L<F> ifs){for(F $ : fs){int b=f($);f($,b);return g($,b,f());}return true;}")//
        .stays();
  }

  @Test public void chocolate3() {
    topDownTrimming("int f(int i){for(int b: fs)return 0;return 1;}").gives("int f(int __){for(int b: fs)return 0;return 1;}")//
        .stays();
  }
  ...
  @Test public void test_aBaCbIfbNullReturnNullABcNewDcdebcdfbIfbgchijbReturnc() {
    topDownTrimming("A<B>a(C b){if(b==null)return null;A<B>c=new D<>();c.d(e(b));c.d(f(b));if(b.g())c.h(i.j(b));return c;}") //
