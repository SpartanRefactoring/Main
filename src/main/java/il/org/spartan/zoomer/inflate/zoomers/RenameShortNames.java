...
/** An expander to rename short or unnecessarily understandable variable names to 
 *  more common or intuitive names (s.e i for an integer variable and ret for a return variable) : <code>
 ...
 * Issue #979 
 * */
public class RenameShortNames extends EagerTipper<MethodDeclaration>{
  ...
  @Override
  @SuppressWarnings("unused") public Tip tip(final MethodDeclaration d, final ExclusionManager __) {
    assert d != null;
    ...
    if(in($.getIdentifier(), "$"))
      return new Tip("Rename paraemter $ to ret", d, getClass()) {
        @Override public void go(final ASTRewrite r, final TextEditGroup g) {
          ...
          //SingleVariableDeclarationAbbreviation.fixJavadoc(d, $, "ret", r, g);
        }
      };
    final SimpleName ¢ = d.getAST().newSimpleName(scope.newName(d, step.type(onlyOne(parameters(d)))));
