...
public class TernaryPushup extends ReplaceCurrentNode<InfixExpression> implements TipperCategory.InVain {
  ...
  @Override public ASTNode replacement(InfixExpression n) {
    ...
    if(conditionalExpression(r)) {
      ConditionalExpression rr = az.conditionalExpression(r);
      InfixExpression le = subject.pair(l, then(rr)).to(operator(n));
      InfixExpression ri = subject.pair(l, elze(rr)).to(operator(n));
      return subject.pair(le, ri).toCondition(expression(rr));
    }
    ConditionalExpression ll = az.conditionalExpression(l);
    InfixExpression le = subject.pair(then(ll), r).to(operator(n));
    InfixExpression ri = subject.pair(elze(ll), r).to(operator(n));
  ...
  @Override public String description(@SuppressWarnings("unused") InfixExpression n) {
    return "";
  }
}
