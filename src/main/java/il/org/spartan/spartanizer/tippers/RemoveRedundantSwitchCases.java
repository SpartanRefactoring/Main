...
public class RemoveRedundantSwitchCases extends CarefulTipper<SwitchStatement> implements TipperCategory.Collapse  {
  @Override public Tip tip(SwitchStatement s) {
    return new Tip(description(s), s, this.getClass()) {
      @Override public void go(ASTRewrite r, TextEditGroup g) {
        @SuppressWarnings("unchecked") List<Statement> l = s.statements();
        ...
        int defaultInd = getDefaultIndex(l);
        if (defaultInd >= 0) {
        ...
        ASTNode node = subject.statement(into.s("switch(" + s.getExpression() + "){" + statementsToString(l) + "}")).toOneStatementOrNull();
        r.replace(s, node, g);
      }
      
      String statementsToString(List<Statement> ss) {
        String $ = new String();
        for(Statement p : ss)
          $ += p;
        return $;
      ...
      int getDefaultIndex(List<Statement> ¢) {
        for(int $ = 0; $ < ¢.size(); ++$)
  ...
  static boolean isListContains(List<Statement> ss, int i, String q) {
