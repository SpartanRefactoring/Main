...
public final class FragmentInitializerStatementTerminatingScope extends $FragementAndStatement //
  ...
  @Override protected ASTRewrite go(final ASTRewrite $, final VariableDeclarationFragment f, final SimpleName n, final Expression initializer,
    ...
    if (haz.annotation(f) || wizard.frobiddenOpOnPrimitive(f, nextStatement) || wizard.isArrayInitWithUnmatchingTypes(f) || initializer == null)
      return null;
    VariableDeclarationStatement parent = az.variableDeclarationStatement(f.getParent());
    if (parent == null)
      return null;
    final Inliner r = Inliner.of(n).by(initializer).in(scope.of(n));
    if (!r.ok())
      return null;
    if (r.addedSize() < savedSize(f))
      if (fragments(parent).size() == 1)
        $.remove(parent, g);
      else
        $.getListRewrite(parent, VariableDeclarationStatement.FRAGMENTS_PROPERTY).remove(f, g);
    return r.fire($, g);
  }

  private static int savedSize(VariableDeclarationFragment ¢) {
    VariableDeclarationStatement $ = az.variableDeclarationStatement(¢.getParent());
    return metrics.size($ != null && fragments($).size() == 1 ? $ : ¢);
  }
}
