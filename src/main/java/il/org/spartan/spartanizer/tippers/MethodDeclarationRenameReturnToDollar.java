...
class Aggressive extends AbstractRenamePolicy {
  ...
  private static int bestScore(final Iterable<SimpleName> ns, final List<ReturnStatement> ss) {
    int $ = 0;
    for (final SimpleName ¢ : ns)
      $ = Math.max($, score(¢, ss));
    return $;
  ...
  private static boolean noRivals(final SimpleName candidate, final Collection<SimpleName> ns, final List<ReturnStatement> ss) {
