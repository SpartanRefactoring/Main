...
public interface idiomatic {
  ...
  @Nullable Trigger tIgnore = new Trigger() {
    @Nullable @Override public <T> T eval(@SuppressWarnings("unused") final Supplier<T> __) {
      return null;
    }
  };
  ...
  class GenericCollector<R> implements Collector<R, Collection<R>, Collection<R>> {
    ...
    @Nullable @Override @SuppressWarnings("unchecked") public Supplier<Collection<R>> supplier() {
      return () -> {
        try {
    ...
    @NotNull @Override public BinaryOperator<Collection<R>> combiner() {
      return (left, right) -> {
        left.addAll(right);
        return left;
      };
    ...
    @NotNull @Override public Function<Collection<R>, Collection<R>> finisher() {
      return castingIdentity();
    }

    @NotNull @Override public Set<Characteristics> characteristics() {
      return new HashSet<>();
    }
  }
}
