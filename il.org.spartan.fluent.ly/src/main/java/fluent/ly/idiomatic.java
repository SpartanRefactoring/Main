...
public interface idiomatic {
	...
	Trigger eval = new Trigger() {
		@Override
		public <T> T eval(final Supplier<T> ¢) {
			return ¢.get();
		}
	};
	/** an ignoring trigger */
	Trigger ignore = new Trigger() {
		@Override
		public <T> T eval(final Supplier<T> ____) {
	...
	/**
	 * A filter, which prints an appropriate log message and returns null in
	 * case of {@link Exception} thrown by {@link Producer#λ()}
	 * 
	 * @param <T>
	 *            JD
	 * @param $
	 *            JD
	 * @return result of invoking the parameter, or <code><b>null</b></code> if
	...
	interface Holder<T> extends Supplier<T> {
		 ...
		 * 
		 * @param unless
		 *            condition on which value is returned
	...
	/**
	 * Evaluate a {@link Runnable} when a condition applies or unless a
	 * condition applies.
	 * 
	 * @author Yossi Gil <Yossi.Gil@GMail.COM>
	 * @since 2016
	...
	class Runner implements Runnable {
		 ...
		 * 
		 * @param unless
		 *            condition n which execution occurs.
		 */
