...
@Utility public class CLASSPATH {
   ...
   * <em>current</em> class path. (The default {@link ClassLoader} fails to detect
   * changes to the class made after system initialization.)
   * <p>
   ...
   * loaded, since it refers to other classes which cannot be found. In this case
   * a {@link NoClassDefFoundError} exception is thrown.
   *
   ...
   *                                some other class it refers to could not be
   *                                found. */
  public static Class<?> initialize(final String className) throws ClassNotFoundException, NoClassDefFoundError {
    return Class.forName(className, true, classLoader());
  }

  /** Mocks {@link Class#forName(String, boolean, ClassLoader)}, where the second
   * parameter is <code><b>false</b></code>, and the third is our local
   * {@link ClassLoader} object. Thus, this function is similar to
   * {@link #initialize(String)}, except that the loaded class is not initialized.
   * <p>
   * <b>Note:</b> Observe that in trying to load a class file, with or without
   * initialization, it could be the case that a certain <tt>.class</tt> class
   * file could be read and is legally structured, it could not be appropriately
