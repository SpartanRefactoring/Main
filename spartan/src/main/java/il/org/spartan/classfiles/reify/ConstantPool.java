...
public final class ConstantPool {
  ...
  public String[] getReferencedMethods() {
    ...
    for (final Constant ¢ : pool)
      if (¢ instanceof MethodReference && !"<init>".equals(((MethodReference) ¢).getNameAndType().getName()))
        $.add(((MethodReference) ¢).getClassConstant().getClassName() + ":"
            + ((MethodReference) ¢).getNameAndType().getName());
    return $.toArray(new String[$.size()]);
  }
  ...
  public class ClassConstant extends Reference {
    ...
    /** What is the fully-qualified class name. In the case of an object array only
     * the class name of the object is returned.
     *
  ...
  public abstract static class Constant {
     ...
     *
     * @return <code><b>true</b></code> <i>iff</i> this instance occupies two 32-bit
     *         words */
  ...
  public static final class Empty extends Literal {
    ...
    /** A default, inaccessible constructor, forcing the singleton restriction */
    private Empty() {
  ...
  /** A representation of an interface method reference constant in the constants'
   * pool
   *
   * @author Yossi Gil */
  ...
  public abstract class MemberReference extends Constant {
    ...
    /** Creates an instance for the specified class, name, and type. Constant pool.
     * Needed for resolving references.
     *
    ...
    @Override public final String toString() {
      return "Class = " + getClassConstant().getClassName() + ", Name = " + getNameAndType().getName()
  ...
  /** An entry in the constant pool which is represented as a reference to another
   * such entry.
   *
   * @author Yossi Gil */
