...
public class ClassProxy<T> {
	 ...
	 * name, even if this name uses a dot character (<code>'.'</code>), instead of a
	 * dollar (<code>'$'</code>) to separate the name of an inner class from the
	 ...
	 * distinction is made by trying to determine whether a class is an inner class
	 * by either loading the class with {@link Class#forName} and if this fails for
	 * some reason, by trying to locate the <tt>.class</tt> file on the file system.
	 * 
	 * @param className a dot separated full class name
	 * @return the inputs, where dots that are found to represent class nesting are
	 *         replaced by dollars.
	 * @see CLASSFILES
	 */
	 ...
	 * {@link Class#getName()}
	 * 
	 * @param className a non-<code><b>null</b></code> representing the class name.
	 */
	...
	/**
	 * Which <code><b>double</b></code>s are found in this class's constants' pool?
	 * 
	 * @return an array with <code><b>double</b></code>s that this class uses.
	 * @throws ClassNotFoundException in case the constants' pool could not be read
	 *                                or found
	 */
	...
	/**
	 * Which <code><b>float</b></code>s are found in this class's constants' pool?
	 * 
	 ...
	 * 
	 * @return an array with all {@link String}s that this class uses @throws
	 * ClassNotFoundException in case the constants' pool could not be read or
	 * found 
	 */
	 ...
	 * @throws ClassNotFoundException in case the constants' pool could not be found
	 */
	...
	private Class<? extends T> retrieveClazz() throws ClassNotFoundException {
		...
		final Class<T> $ = (Class<T>) findClass(className);
		return $;
	}
