...
public class BitSetSmallIntegersGraph extends SmallIntegersGraph {
  ...
  private BitSetSmallIntegersGraph(final BitSet[] neighbors, final int arcsCount, final short[] component,
      final short[] nodes) {
  ...
  public static class TEST {
    ...
    @Test public void hasThreeNeighbors() {
      azzert.that(new Builder().connect(13, 14).connect(13, 15).connect(13, 12).go().neighbors(13).cardinality(),
          is(3));
    }
    ...
    @Test public void triangleAndPath() {
      azzert.that(new Builder().connect(1, 2).connect(2, 3).connect(3, 4).connect(7, 8).connect(5, 6).connect(6, 7).go()
          .components(), is(2));
    }
