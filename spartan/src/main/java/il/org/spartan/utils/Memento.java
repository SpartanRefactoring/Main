...
@Instantiable public class Memento<T> {
  ...
  public T restore() {
    try {
      @SuppressWarnings("unchecked") final T $ = (T) new ObjectInputStream(new ByteArrayInputStream(snapshot))
          .readObject();
      return $;
   ...
   * @return <code><b>true</b></code>, <i>iff</i> the given object is deeply equal
   *         to the snapshot
   ...
   * @return a byte array snapshot of the parameter as obtained by serializing it
   * @throws Bug.Contract.Precondition in case object could not be serialized */
