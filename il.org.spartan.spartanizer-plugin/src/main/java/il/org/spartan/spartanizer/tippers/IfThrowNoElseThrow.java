...
public final class IfThrowNoElseThrow extends GoToNextStatement<IfStatement>//
  ...
  static Expression getThrowExpression(final Statement ¢) {
    final ThrowStatement ret = extract.throwStatement(¢);
    return ret == null ? null : extract.core(ret.getExpression());
  }
  ...
  @Override protected ASTRewrite go(final ASTRewrite r, final IfStatement s, final Statement nextStatement, final TextEditGroup g) {
    if (!iz.vacuousElse(s))
      return null;
    final Expression ret = getThrowExpression(then(s));
    if (ret == null)
      return null;
    final Expression e2 = getThrowExpression(nextStatement);
    return e2 == null ? null : misc.replaceTwoStatements(r, s, subject.operand(subject.pair(ret, e2).toCondition(s.getExpression())).toThrow(), g);
  }
}
