...
public abstract class InfixExpressionSortingFull extends InfixExpressionSorting {
  ...
  private static boolean sideEffectSafe(List<Expression> $) {
  ...
  static boolean literalsExpression(Expression ¢){
    return iz.literal(¢) || (iz.infixExpression(¢) && extract.allOperands(az.infixExpression(¢)).stream().allMatch(InfixExpressionSortingFull::literalsExpression));
  }
