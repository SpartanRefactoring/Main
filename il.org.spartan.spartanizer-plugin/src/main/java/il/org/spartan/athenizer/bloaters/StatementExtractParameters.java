...
public class StatementExtractParameters<S extends Statement> extends CarefulTipper<S>//
	...
	@Override
	public String description(@SuppressWarnings("unused") final Statement __) {
		return "Extract complex parameter from statement";
	}

	@Override
	public Tip tip(final Statement s) {
		...
		final Collection<String> createdImports = getCreatedImports(ir), sameFile = getSameFile(createdImports, types),
				samePackage = getSamePackage(createdImports, types[0]),
				topLevel = getTopLevel(createdImports, topTypes);
				...
				nonPublicImportHazard(allBindings, samePackage) ? null : //
						new Tip(description(s), myClass(), s) {
							...
							public void go(final ASTRewrite r, final TextEditGroup g) {
								fixAddedImports(s, createdImports, sameFile, samePackage, topLevel, g,
										r.getListRewrite(u, CompilationUnit.IMPORTS_PROPERTY));
								...
								s.subtreeMatch(
										new ASTMatcherSpecific($, λ -> r.replace(λ, make.from(s).identifier(nn), g)),
										ns);
							...
							void goNonBlockParent(final ASTNode p, final VariableDeclarationStatement x,
									final Statement pleaseDoNotChangeThisVariableNameToSItCausesAHidingBug,
							...
							void goBlockParent(final Block b,
									final VariableDeclarationStatement pleaseDoNotChangeThisVariableNameToSItCausesAHidingBug,
									final Statement pleaseDoNotChangeThisVariableNameToSItCausesAHidingBug2,
									final ASTRewrite r, final TextEditGroup g) {
	 ...
	 */
	static void fixAddedImports(final Statement s, final Collection<String> createdImports,
			final Collection<String> sameFile, final Collection<String> samePackage, final Collection<String> topLevel,
	...
	static Type fixWildCardType(final Type $) {
		...
		$.accept(new ASTVisitor(true) {
			...
			@Override
			public boolean visit(@SuppressWarnings("hiding") final WildcardType $) {
				if (s.indexOf($ + "") != 0)
					return super.visit($);
				stop = true;
	...
	static boolean isComplicated(final Expression ¢) {
		return iz.nodeTypeIn(¢, CLASS_INSTANCE_CREATION, METHOD_INVOCATION, INFIX_EXPRESSION, ASSIGNMENT,
				CONDITIONAL_EXPRESSION, LAMBDA_EXPRESSION);
	}
	...
	private static boolean ambiguousImports(final Collection<String> createdImports, final CompilationUnit u) {
		...
		if (property.has(u, KNOWN_TYPES_NAMES))
			...
			u.accept(new ASTVisitor() {
				...
				@Override
				public boolean visit(final SimpleName ¢) {
		...
		return !Collections.disjoint(
				createdImports.stream().map(λ -> λ.split("\\.")).map(λ -> λ[λ.length - 1]).collect(Collectors.toList()),
				usedNames);
	}

	private static boolean privateImportHazard(final List<ITypeBinding> allBindings,
			final Collection<String> sameFile) {
		return allBindings.stream()
				.anyMatch(λ -> sameFile.contains(λ.getQualifiedName()) && Modifier.isPrivate(λ.getModifiers()));
	}

	private static boolean nonPublicImportHazard(final List<ITypeBinding> allBindings,
			final Collection<String> samePackage) {
		return allBindings.stream()
				.anyMatch(λ -> samePackage.contains(λ.getQualifiedName()) && !Modifier.isPublic(λ.getModifiers()));
	}
	...
	protected class ASTMatcherSpecific extends ASTMatcher {
		...
		@Override
		public boolean match(final CreationReference node, final Object other) {
			if (node != toMatch || !(other instanceof ASTNode))
				return super.match(node, other);
			onMatch.accept((ASTNode) other);
			return false;
		...
		@Override
		public boolean match(final Dimension node, final Object other) {
			if (node != toMatch || !(other instanceof ASTNode))
				return super.match(node, other);
			onMatch.accept((ASTNode) other);
			return false;
		...
		@Override
		public boolean match(final ExpressionMethodReference node, final Object other) {
			if (node != toMatch || !(other instanceof ASTNode))
				return super.match(node, other);
			onMatch.accept((ASTNode) other);
			return false;
		...
		@Override
		public boolean match(final IntersectionType node, final Object other) {
			if (node != toMatch || !(other instanceof ASTNode))
				return super.match(node, other);
			onMatch.accept((ASTNode) other);
			return false;
		...
		@Override
		public boolean match(final LambdaExpression node, final Object other) {
			if (node != toMatch || !(other instanceof ASTNode))
				return super.match(node, other);
			onMatch.accept((ASTNode) other);
			return false;
		...
		@Override
		public boolean match(final NameQualifiedType node, final Object other) {
			if (node != toMatch || !(other instanceof ASTNode))
				return super.match(node, other);
			onMatch.accept((ASTNode) other);
			return false;
		...
		@Override
		public boolean match(final SuperMethodReference node, final Object other) {
			if (node != toMatch || !(other instanceof ASTNode))
				return super.match(node, other);
			onMatch.accept((ASTNode) other);
			return false;
		...
		@Override
		public boolean match(final TypeMethodReference node, final Object other) {
			if (node != toMatch || !(other instanceof ASTNode))
				return super.match(node, other);
			onMatch.accept((ASTNode) other);
			return false;
