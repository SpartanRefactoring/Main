...
class AssertJTransformator implements TestTransformator {
    ...
    static Transformation[] transformations = { //
            ...
            // language=RegExp
            from("assertEquals\\((.*),(.*).size\\(\\)\\);").to("assertThat\\(%s\\).hasSize\\(%s\\);")
                    .reorder(1, 0),
            // language=RegExp
            from("assertEquals\\((.*),(.*)\\);").to("assertThat\\(%s\\).isEqualTo\\(%s\\);")
                    .reorder(1, 0),
            // language=RegExp
            from("assertNotEquals\\((.*),(.*)\\);").to("assertThat\\(%s\\).isNotEqualTo\\(%s\\);")
                    .reorder(1, 0),
            // language=RegExp
            ...
            // language=RegExp
            from("assert (.*) <= (.*) && (.*) <= (.*);").to("assertThat\\(%s\\).isBetween\\(%s, %s\\);")
                    .reorder(1, 0, 3),
            // language=RegExp
