...
public class ASTTestClassGenerator implements TestClassGenerator {
    ...
    /**
     * Creates infixes of tests.
     * Each test with 2n lines is duplicated into:
     * - n tests where each line is asserted only once.
     * - n tests (maximum) where each lines is null chcked only once (if the line had a dereference expression in it).
     *
     * @param ¢ a test object. (that contains a code of aa method annotated with a JUnit 4 @Test)
     * @return the new tests as strings.
     */
    private static List<String> infixes(final Test ¢) {
        final List<String> allStatements = step.statements((MethodDeclaration) ¢.source).stream()
                .map(Object::toString)
                .map(TestTransformator::transformIfPossible)
                .flatMap(s -> statementsWithNullChecks(s).stream())
                .collect(Collectors.toList());
        final Int i = new Int(0);
        return allStatements.stream()
                .map(s -> new Pair<>(i.next(), copyOf(allStatements)))
                .map(p -> shutDown(p.second, p.first))
                .map(l -> l.stream()
                        .map(s -> s.trim())
                        .filter(s -> !s.matches("[\n\r\t ;]*"))
                        .reduce((s1, s2) -> s1 + "\n" + s2).orElse("")).collect(Collectors.toList());
    }

    /**
     * Adds null checks for dereferencing objects.
     *
     * @param s line of code
     * @return null checked and un-null checked version of s
     */
    private static List<String> statementsWithNullChecks(String s) {
        return compute.usedIdentifiers(makeASTNode(s))
                .map(i -> String.format(ASSERT_NOT_NULL_TEMPLATE, i))
     ...
     *
     * @param testClassName      the meta test name (including the _Meta suffix)
     * @param originalSourceFile the test file
    ...
    public Class<?> generate(final String testClassName, final File originalSourceFile) {
        ...
        final String $ = generateNewClassString(allTestMethods().stream().map(λ -> tests(λ, infixes(λ))).collect(Collectors.toList()));
     ...
     *
     * @param originalSourceFile java file
    ...
    private void removeUnnecessaryImports() {
        ...
        root.accept(new ASTVisitor() {
            ...
            public boolean visit(final ImportDeclaration ¢) {
                if (importsToRemove.stream().map(x -> x.getPackage().getName()).anyMatch(x -> ¢.getName().toString().contains(x))
                        || ¢.getName().toString().equals("org.junit.runner")) {
                    ¢.delete();
                    return true;
                }
                return false;
    ...
    /**
     * Generates a list of @link{{@link Test Test} objects containing the original tests.}
     *
    ...
    private List<Test> allTestMethods() {
        ...
        root.accept(new ASTVisitor() {
            ...
            public boolean visit(final MethodDeclaration ¢) {
                final Annotation a = extract.annotations(¢).stream()
                        .filter(λ -> haz.name(λ, "Test"))
                        .findAny().orElse(null);
                if (a != null) {
    ...
    /**
     * Modifying the root of the AST to adapt it to a MetaTest format:
     * - Adding the apropriate static imports of the external testing libraries (AssertJ / Hamcrest ...)
     * - Adding "_Meta" suffix to the test's name
     * - Removing the "@RunWith(MetaTester.class)" annotation to prevent from rerunning this code to infinity.
     *
     * @return the root after the modifications.
     */
    private ASTNode modifyAST() {
        root = removeOriginalTestsFromTree();
        root = addStaticImports(root, document, ASSERTJ_STATIC_IMPORT_STATEMENT, ASSERTJ_STATIC_IMPORT_STATEMENT_SIMPLE_NAME);
    ...
    /**
     * Removing the original tests from root.
     * The new tests will be inserted afterwards instead.
     *
     * @return root.
     */
    ...
    /**
     * Returns new test method name in the format:
     * [test name]_meta[i]
     * where i is the index of the method in the new meta test file.
     *
