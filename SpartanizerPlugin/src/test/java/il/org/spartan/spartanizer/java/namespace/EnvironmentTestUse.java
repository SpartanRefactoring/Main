...
public class EnvironmentTestUse {
  ...
  @Test public void useTestUsesAndDefinitions3() {
    ...
    assert $.contains("x") == true;
    assert $.contains("y") == true;
    assert $.contains("z") == true;
  }

  @Test public void useTestWithDefinitionsOnly() {
    assert Environment.uses(makeAST.COMPILATION_UNIT.from(new Document("int x = 5;").get())).contains("x") == true;
  }
  ...
  @Test public void useTestWithUsesOnly2() {
    assert Environment.uses(makeAST.COMPILATION_UNIT.from(new Document("foo(x)").get())).contains("x") == true;
  }
  ...
  @Test public void useTestWithUsesOnly5() {
    assert Environment.uses(makeAST.COMPILATION_UNIT.from(new Document("x.foo()").get())).contains("x") == true;
  }
}
