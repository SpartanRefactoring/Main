...
public class Issue0404 {
  ...
  @Test public void ka() {
    ...
    assert the.first(dig.stringLiterals(into.e("\"\""))) != null
        && the.first(dig.stringLiterals(into.e("\"\""))).isEmpty() : "The contained element was not the expected one.";
  }

  @Test public void kb() {
    assert dig.stringLiterals(into.e("\"str\"")).size() == 1 : "The List did not contain the expected number of elements.";
    assert "str".equals(the.first(dig.stringLiterals(into.e("\"str\"")))) : "The contained element was not the expected one.";
  }

  @Test public void kc() {
    final List<String> $ = dig.stringLiterals(into.a("s = \"a\""));
    assert $.size() == 1 : "The List did not contain the expected number of elements.";
    assert "a".equals(the.first($)) : "The contained element was not the expected one.";
  }

  @Test public void kd() {
    final List<String> $ = dig.stringLiterals(into.c("\"a\".size()> b.size() ? b : a"));
    assert $.size() == 1 : "The List did not contain the expected number of elements.";
    assert "a".equals(the.first($));
  }
  ...
  @Test public void lf() {
    ...
    assert "first".equals(the.first($)) : "List did not contain expected element \"first\" at index 0";
  ...
  @Test public void mb() {
    ...
    assert $.size() == 2 : "The List did not contain the expected number of elements.";
    assert "\"".equals(the.first($)) : "The List did not contain the expected element \" at index 0";
  ...
  @Test public void na() {
    final List<String> $ = dig.stringLiterals(into.cu(//
        "class A{\n"//
            ...
            + "}"));
    assert $.size() == 4 : "The List did not contain the expected number of elements.";
    assert "ĀĆ".equals(the.first($)) : "List did not contain expected element \"ĀĆ\" at index 0";
  ...
  @Test public void nb() {
    final List<String> $ = dig.stringLiterals(into.cu(//
        "class A{\n"//
            ...
            + "}"));
    assert $.size() == 4 : "The List did not contain the expected number of elements.";
    assert "עוד חוזר הניגון שזנחת לשווא".equals(the.first($)) : "List did not contain expected element \"עוד חוזר הניגון שזנחת לשווא\" at index 0";
