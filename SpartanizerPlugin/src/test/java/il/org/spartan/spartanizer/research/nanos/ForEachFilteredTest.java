...
public class ForEachFilteredTest {
  ...
  @Test public void d() {
    trimmingOf("for(A r :(B)rs) if(U.t(r.tr())) try{r.es(); }catch(Throwable ¢){l.ac(\"ma\",¢); }")//
        ...
        .gives("((B)rs).stream().filter(¢->U.t(¢.tr())).forEach(¢->{try{¢.es();}catch(Throwable ¢){{l.ac(\"ma\",¢);}}});")//
        .gives("((B)rs).stream().filter(¢->U.t(¢.tr())).forEach(¢->{try{¢.es();}catch(Throwable ¢){l.ac(\"ma\",¢);}});")//
        .stays();
  }

  @Test public void e() {
    trimmingOf("for(final D ¢ : s.th(ref)) if(!¢.isC() && !is.st¢(¢) && !iz.final¢(¢) && !iz.private¢(¢)) put(mangle(¢), ¢);")//
        .using(EnhancedForStatement.class, new ForEachSuchThat())//
        .gives("(s.th(ref)).stream().filter(¢->!¢.isC()&&!is.st¢(¢)&&!iz.final¢(¢)&&!iz.private¢(¢)).forEach(¢->put(mangle(¢),¢));")//
        .stays();
  }
}
