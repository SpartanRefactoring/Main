...
public final class MethodExplorerTest {
 ...
 azzert.that(
 new MethodExplorer(into.d(" void f() { try { f(); } catch (final Exception|RuntimeException e) { f(); } }"))
 .localVariables().size(),
 is(1));
 }
 ...
 azzert.that(
 new MethodExplorer(into.d(" int sum(final int is[]) { int $ = 0; for (final int i : is) $ += i; return $; } "))
 .localVariables().size(),
 is(2));
 }
