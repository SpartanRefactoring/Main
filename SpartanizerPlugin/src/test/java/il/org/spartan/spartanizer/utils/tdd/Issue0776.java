...
public class Issue0776 {
  ...
  @Test public void checkSyncronize() {
    azzert.that(1, is(enumerate.blockTypes((MethodDeclaration) wizard.ast("public void addName(String name){int x=0; synchronized(this) {++x;}}"))));
  }
  ...
  @Test public void lambadaExprCheck() {
    azzert.that(1, is(enumerate
        .blockTypes((MethodDeclaration) wizard.ast("public void addName(String name){Runnable r = () -> {System.out.println(\"hello world\");};}"))));
  }

  @Test public void tryChatch() {
    azzert.that(1,
        is(enumerate.blockTypes((MethodDeclaration) wizard.ast("public void addName(String name){try {} catch (IndexOutOfBoundsException e) {}}"))));
  }
}
