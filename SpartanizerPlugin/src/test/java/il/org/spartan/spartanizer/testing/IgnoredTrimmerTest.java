...
public final class IgnoredTrimmerTest {
  public void doNotInlineDeclarationWithAnnotationSimplified() {
    topDownTrimming("@SuppressWarnings() int $ = (Class<T>) findClass(className);\n")//
        .stays();
  }
  ...
  @Test public void inlineSingleUse01() {
    topDownTrimming("/* * This is a comment */ int i = y(0); int j = 3; int k = j+2; int m = k + j -19; y(m*2 - k/m + i); ")
        .gives("/* * This is a comment */ int j = 3; int k = j+2; int m = k + j -19; y(m*2 - k/m + (y(0))); ");
  }

  @Test public void inlineSingleUse02() {
    topDownTrimming("/* * This is a comment */ int i = 5,j=3; int k = j+2; int m = k + j -19 +i; y(k); ")
  ...
  @Test public void inlineSingleUse04() {
    topDownTrimming("int x = 6; final C b = new C(x); int y = 2+b.j; y(y-b.j); y(y*2); ")
        .gives("final C b = new C((6)); int y = 2+b.j; y(y-b.j); y(y*2); ");
  }

  @Test public void inlineSingleUse05() {
    topDownTrimming("int x = 6; final C b = new C(x); int y = 2+b.j; y(y+x); y(y*x); ")//
  ...
  @Test public void reanmeReturnVariableToDollar01() {
    topDownTrimming("public C(int i) { j = 2*i; public final int j; public C yada6() { final C res = new C(6); S.x.f(res.j); return res; ")
  ...
  @Test public void renameVariableUnderscore2() {
    topDownTrimming("class A {int __; int f(int __) {return __;}}")//
  ...
  @Test public void replaceClassInstanceCreationWithFactoryClassInstanceCreation() {
    topDownTrimming("Character x = new Character(new Character(f()));")//
        .gives("Character x = Character.valueOf(Character.valueOf(f()));");
  }

  @Test public void stringFromBuilderAddParenthesis() {
    topDownTrimming("new StringBuilder(f()).append(1+1).toString()")//
        .gives("\"\" + f() + (1+1)");
  }

  @Test public void stringFromBuilderGeneral() {
    topDownTrimming("new StringBuilder(myName).append(\"\'s grade is \").append(100).toString()")//
        .gives("myName + \"\'s grade is \" + 100");
  }

  @Test public void stringFromBuilderNoStringComponents() {
    topDownTrimming("new StringBuilder(0).append(1).toString()")//
        .gives("\"\" + 0 + 1");
  }
}