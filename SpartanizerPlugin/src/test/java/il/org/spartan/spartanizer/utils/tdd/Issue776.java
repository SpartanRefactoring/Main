...
@SuppressWarnings("static-method") public class Issue776 {
  ...
  @Test public void checkWhileAndForBlocks() {
    assertEquals(enumerate.blockTypes(
        ((MethodDeclaration) wizard.ast("public int foo(int x)" + "{{} {} if (false) {} while(x!=0) {} for(int i=0;i<5;i++){x=7;}}"))), 4);
  }
  ...
  @Test public void tryChatch() {
    assertEquals(enumerate
        .blockTypes(((MethodDeclaration) wizard.ast("public void addName(String name)" + "{try {} catch (IndexOutOfBoundsException e) {}}"))), 1);
  }
  @Test public void lambadaExprCheck() {
    assertEquals(
        enumerate.blockTypes(
