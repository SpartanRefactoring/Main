...
public class Issue0753 {
  ...
  @Test public void e() {
    azzert.that("foo", is(first(getAll.methods(az.compilationUnit(make.ast("class A{boolean foo(){return false;}}")))).getName().getIdentifier()));
  }
  ...
  @Test public void f() {
    ...
    azzert.that("foo",
        is(first(getAll.methods(az.compilationUnit(make.ast("public class A {void foo(/*lololo*/ ){            } }")))).getName().getIdentifier()));
