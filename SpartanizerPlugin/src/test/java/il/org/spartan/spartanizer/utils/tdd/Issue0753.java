...
public class Issue0753 {
  ...
  @Test public void e() {
    azzert.that("foo", is(the.first(getAll.methods(az.compilationUnit(make.ast("class A{boolean foo(){return false;}}")))).getName().getIdentifier()));
  }
  ...
  @Test public void f() {
    ...
    azzert.that("foo",
        is(the.first(getAll.methods(az.compilationUnit(make.ast("public class A {void foo(/*lololo*/ ){            } }")))).getName().getIdentifier()));
    azzert.that("elite", is(the.first(res2).getName().getIdentifier()));
