...
public class MergeCatches extends ReplaceCurrentNode<TryStatement> implements TipperCategory.Collapse {
  @SuppressWarnings("unchecked") @Override public ASTNode replacement(TryStatement s) {
    List<CatchClause> lst = step.extractCatches(s);
    for (int i = 0; i < lst.size(); ++i)
      for (int j = i + 1; j < lst.size(); ++j)
        if (wizard.same(lst.get(i).getBody(), lst.get(j).getBody())) {
          TryStatement $ = duplicate.of(s);
          CatchClause mergedCatch = duplicate.of(lst.get(i));
          $.catchClauses().remove(i);
          $.catchClauses().remove(j - 1);
          UnionType ut = s.getAST().newUnionType();
  ...
  @Override public String description(TryStatement ¢) {
