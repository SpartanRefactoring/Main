...
public class SameEvaluationConditional extends ReplaceCurrentNode<ConditionalExpression> implements TipperCategory.Collapse{
  ...
  @Override public ASTNode replacement(ConditionalExpression x) {
    InfixExpression cond = az.infixExpression(x.getExpression());
    return !iz.infixEquals(cond) || !wizard.same(x.getThenExpression(), cond.getLeftOperand()) && !wizard.same(x.getThenExpression(), cond.getRightOperand())
  ...
  @Override public String description(@SuppressWarnings("unused") ConditionalExpression ¢) {
