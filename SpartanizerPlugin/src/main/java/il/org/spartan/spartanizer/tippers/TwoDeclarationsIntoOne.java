...
public class TwoDeclarationsIntoOne extends ReplaceToNextStatement<VariableDeclarationStatement> implements TipperCategory.Abbreviation {
  @Override @SuppressWarnings("unchecked") protected ASTRewrite go(ASTRewrite $, VariableDeclarationStatement s, Statement nextStatement,
      TextEditGroup g) {
    if (!canTip(s, nextStatement))
      return null;
    VariableDeclarationStatement ns = (VariableDeclarationStatement) nextStatement, sc = copy.of(s);
    for (VariableDeclarationFragment ¢ : (List<VariableDeclarationFragment>) ns.fragments())
  ...
  private static boolean canTip(VariableDeclarationStatement $, Statement nextStatement) {
