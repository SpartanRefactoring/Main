...
/** examples in {@link #examples}
 * test case is {@link Issue1159}
...
public class SwitchAddDefault extends SwitchZtatement implements TipperCategory.Bloater {
  ...
  @Override public Example[] examples() {
    return new Example[] {
      convert("switch(x) { case 1: y=2; break; }").to("switch(x) { case 1: y=2; break; default: }"),
      ignores("switch(x) { case 1: y=2; break; default: }")
    };
  }
  
  public SwitchAddDefault() {
    andAlso(Proposition.of("Does not have default case", () -> 
      (cases().stream().filter(λ->λ.isDefault()).collect(Collectors.toList()).isEmpty())));
  }
  
  @Override protected ASTRewrite go(ASTRewrite $, TextEditGroup g) {
    ListRewrite l = $.getListRewrite(current, SwitchStatement.STATEMENTS_PROPERTY);
    SwitchCase ss = current.getAST().newSwitchCase();
