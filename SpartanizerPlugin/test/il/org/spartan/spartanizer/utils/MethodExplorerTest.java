...
public final class MethodExplorerTest {
  @Test public void localVariablesCatchExpression() {
    azzert.that(new MethodExplorer(into.d(" void f() { try { f(); } catch (final Exception|RuntimeException e) { f(); } }")).localVariables().size(),
        is(1));
  }

  @Test public void localVariablesExtendedForLoop() {
    azzert.that(
        new MethodExplorer(into.d(" int sum(final int is[]) { int $ = 0; for (final int i : is) $ += i; return $; } ")).localVariables().size(),
        is(2));
  }

  @Test public void localVariablesForLoopNoVariable() {
    azzert.that(new MethodExplorer(into.d(" int f() { for (f(); i*j <10; i += j++); }")).localVariables().size(), is(0));
  }

  @Test public void localVariablesForLoopOneVariable() {
    azzert.that(new MethodExplorer(into.d(" int f() { for (int i = 0; i*j <10; i += j++); }")).localVariables().size(), is(1));
  }

  @Test public void localVariablesForLoopTwoVariables() {
    azzert.that(new MethodExplorer(into.d(" int f() { for (int i = 0, j = 2; i*j <10; i += j++); }")).localVariables().size(), is(2));
  }

  @Test public void localVariablesMultipleFragments() {
    azzert.that(new MethodExplorer(into.d(" int f() { int a,b; }")).localVariables().size(), is(2));
  }

  @Test public void localVariablesMultipleNestedFragments() {
    azzert.that(new MethodExplorer(into.d(" int f() { int a,b; {int c, d;}}")).localVariables().size(), is(4));
  }
  ...
  @Test public void localVariablesRepeatedNestedFragments() {
    azzert.that(new MethodExplorer(into.d(" int f() { int a,b,c,d; {int i, j;} {int i,j; int k;}")).localVariables().size(), is(9));
  }
  ...
  @Test public void localVariablesVanilla() {
    azzert.that(new MethodExplorer(into.d(" int f() { int a; }")).localVariables().size(), is(1));
  }
