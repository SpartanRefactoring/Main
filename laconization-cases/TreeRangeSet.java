...
public class TreeRangeSet<C extends Comparable<?>> extends AbstractRangeSet<C>
  ...
  @Override
  public boolean intersects(Range<C> c) {
    checkNotNull(c);
    Entry<Cut<C>, Range<C>> ceilingEntry = rangesByLowerBound.ceilingEntry(c.lowerBound);
    if (ceilingEntry != null
        && ceilingEntry.getValue().isConnected(c)
        && !ceilingEntry.getValue().intersection(c).isEmpty())
		return true;
    Entry<Cut<C>, Range<C>> priorEntry = rangesByLowerBound.lowerEntry(c.lowerBound);
    return priorEntry != null
        && priorEntry.getValue().isConnected(c)
        && !priorEntry.getValue().intersection(c).isEmpty();
  }
    ...
    Iterator<Entry<Cut<C>, Range<C>>> entryIterator() {
      ...
      if (!upperBoundWindow.hasLowerBound())
        ...
        backingItr = ((lowerEntry == null ? rangesByLowerBound
				: rangesByLowerBound.tailMap(upperBoundWindow.lowerBound.isLessThan(lowerEntry.getValue().upperBound)
						? lowerEntry.getKey() : upperBoundWindow.lowerEndpoint(), true)).values()).iterator();
      }
      return new AbstractIterator<Entry<Cut<C>, Range<C>>>() {
        ...
        protected Entry<Cut<C>, Range<C>> computeNext() {
          ...
          return upperBoundWindow.upperBound.isLessThan(range.upperBound) ? endOfData()
      ...
      return new AbstractIterator<Entry<Cut<C>, Range<C>>>() {
        ...
        protected Entry<Cut<C>, Range<C>> computeNext() {
          if (!backingItr.hasNext())
			return endOfData();
          Range<C> range = backingItr.next();
          return !upperBoundWindow.lowerBound.isLessThan(range.upperBound) ? endOfData()
				: Maps.immutableEntry(range.upperBound, range);
        }
      };
    }
  ...
  private static final class ComplementRangesByLowerBound<C extends Comparable<?>>
    ...
    Iterator<Entry<Cut<C>, Range<C>>> descendingEntryIterator() {
       ...
       */
      Cut<C> startingPoint =
          !complementLowerBoundWindow.hasUpperBound() ? Cut.<C>aboveAll() : complementLowerBoundWindow.upperEndpoint();
      boolean inclusive =
  ...
  private static final class SubRangeSetRangesByLowerBound<C extends Comparable<?>>
    ...
    private NavigableMap<Cut<C>, Range<C>> subMap(Range<Cut<C>> window) {
      return !window.isConnected(lowerBoundWindow) ? ImmutableSortedMap.of()
			: new SubRangeSetRangesByLowerBound<C>(lowerBoundWindow.intersection(window), restriction,
					rangesByLowerBound);
    }
      ...
      return new AbstractIterator<Entry<Cut<C>, Range<C>>>() {
        ...
        protected Entry<Cut<C>, Range<C>> computeNext() {
          ...
          if (restriction.lowerBound.compareTo(nextRange.upperBound)>= 0)
			return endOfData();
          nextRange = nextRange.intersection(restriction);
          return !lowerBoundWindow.contains(nextRange.lowerBound) ? endOfData()
